...
static void normalize_albumart_path(char *dst, size_t dst_sz, const char *src) {
    ...
    // Keep this program local-only. If Volumio gives us Spotify/CDN HTTPS art,
    // ask Volumio's local albumart endpoint to proxy it instead of adding HTTPS libs.
    if (strncmp(src, "http://", 7) == 0 || strncmp(src, "https://", 8) == 0) {
        snprintf(dst, dst_sz, "/albumart?web=%s", src);
        return;
    }
