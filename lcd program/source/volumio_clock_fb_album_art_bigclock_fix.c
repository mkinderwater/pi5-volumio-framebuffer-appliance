...
static bool albumart_is_real_track_art(void) {
    ...
    /*
      Volumio may report generic /albumart while it is waiting or between tracks.
      That should not count as useful track metadata, otherwise the timeout never
      returns the screen to the large idle clock.
    */
    if (strcmp(g_status_albumart, "/albumart") == 0) return false;

    /*
      Real track art normally carries cache/path/web/query details.
      Local file paths are also allowed if they are more specific than /albumart.
    */
