...
static bool create_default_json_config(void) {
    ...
    const char *json_text = json_object_to_json_string_ext(root, JSON_C_TO_STRING_PRETTY);
        ...
        for (unsigned int row = 0; row < bitmap->rows; row++) {
            for (unsigned int col = 0; col < bitmap->width; col++) {
                ...
                if (alpha == 255) {
                    ...
                    // Fast Exact Division by 255 Cache Op
                    g_img[idx + 0] = fast_div_255((uint16_t)alpha * r_text + inv * g_img[idx + 0]);
                    g_img[idx + 1] = fast_div_255((uint16_t)alpha * g_text + inv * g_img[idx + 1]);
                    g_img[idx + 2] = fast_div_255((uint16_t)alpha * b_text + inv * g_img[idx + 2]);
                }
            }
        }
...
static bool draw_clock(bool *out_needs_scroll, bool *out_layout_animating) {
    ...
    *out_layout_animating = false;
    ...
    // Heartbeat colon for the large idle clock only.
    // Do not replace ':' with a space. Space and colon have different glyph widths,
    // which makes the minute digits move. Instead, draw the colon in very dark grey.
    ...
    if (big_clock_heartbeat) {
        ...
        if (colon) {
            ...
            uint32_t colon_color = ((now_tm.tm_sec % 2) != 0) ? 0x080808 : TEXT_COLOR_MAIN;

            draw_text(g_font_bold, hour_part, cur_clock_x, cur_clock_y, TEXT_COLOR_MAIN);
            draw_text(g_font_bold, ":", colon_x, cur_clock_y, colon_color);
        ...
        bool cpu_temp_hot = format_cpu_temp_text(temp_str, sizeof(temp_str));
